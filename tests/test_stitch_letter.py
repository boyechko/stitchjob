...
def copy_data_module(tmp_path_factory):
    """Copy test/data/ into a temp directory once per test module."""
...
def test_stitch_letter_to_tex(test_data_session):
    output = subprocess.run(["python3", "stitch_letter.py", test_data_session / "letter.md", "-r", test_data_session / "resume.xml"])
...
def test_stitch_letter_to_pdf(test_data_session):
    output = subprocess.run(["python3", "stitch_letter.py", test_data_session / "letter.md", "-r", test_data_session / "resume.xml", "-p"])
    assert (test_data_session / "letter.pdf").exists()

def test_stitch_letter_signature_image(test_data_session):
    output = subprocess.run(["python3", "stitch_letter.py", test_data_session / "letter.md", "-r", test_data_session / "resume.xml", "-s", "-S", test_data_session / "signature.png"])
    tex_path = test_data_session / "letter.tex"
    assert tex_path.exists()
